...
date: 2025-11-02
params:
  bluesky: https://bsky.app/profile/somas.is/post/3m4pgyetnus25
aliases:
...
do not necessarily have to correspond to their file's tags.[^1]
...
1. fetches metadata from sources like MusicBrainz, etc. by
   ...
   c. and finally, asking you to pick a matching album from those sources[^noautotag]
...
2. Then, once the file importing really starts,
   ...
   c. sets the fields for each file record based on the file's tags,[^fields-from-tags]
...
that you made to the file tags.[^surprised]
...
for any changes from the metadata source used during importing.[^mbsync]
...
If memory serves,
among all the different music players I've tried over the years,[^players]
...
[^players]:
    ...
    [Audacious],
    [cmus] (for a time),
    [Noise] (excuse me, I guess it's Pantheon Music now;
    it was Noise back when I used it),
...
I would be lying if I said I was completely satisfied with it,[^elisa]
