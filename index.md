...
- In early 2021, I hit the big time and [got to make a set for WNYU][music-wnyu],
  ...
  a song by another well known artist in the pony music
  scene](https://somasis.bandcamp.com/track/season-infinity-replacer-cover).
  And then...​ I lost a lot of steam, finally, and started focusing more
  on my philosophy degree.
- In mid 2020 [I did an unofficial remix of a Sweet Trip song I
  liked](https://somasis.bandcamp.com/track/dedicated-somasis-remix).
  Later that year [I worked on a song with an idol of
  mine](https://somasis.bandcamp.com/track/starsailor). In late 2020, I
  put out [a companion
  EP](https://somasis.bandcamp.com/album/in-nature-together) to my
  previous album ([spirit / songs for
  lovers](https://somasis.bandcamp.com/album/spirit-songs-for-lovers))
  that I still like. At the tail end of 2020, [I covered a song by an
  idol of
  mine](https://somasis.bandcamp.com/album/dashy-hearths-warming-eve),
...
- In 2020, I got bored and wrote a reimplementation of
  ...
  [`passage`](https://github.com/somasis/passage), and it was superceded
  anyway (same name even!) by a more recent project doing the same thing
  by the author of `age`.
- In 2019, [I made my first real
  single](https://somasis.bandcamp.com/track/january-2), and then my
  first Real Album, [\"spirit / songs for
  lovers\"](https://somasis.bandcamp.com/album/spirit-songs-for-lovers),
  which might be the only thing you've heard by me. A few months later,
  it got a little buzz and it gave me way too much of an ego boost and I
...
- In 2017-2020, some time after I stopped actively developing for
  ...
  pages on the system; `s6` was to be the primary `init(8)` system, and
  the administrator would interact with system services, user services,
  a user's X session, and timed jobs using an `s6` frontend called
  `commune`; `snooze` would be used rather than `crond`, since it
  ...
  preferred standard C library, `toybox` would provide coreutils,
  ...
  biggest timesinks, and the extreme amount of flexibility I shot for in
  the system packaging was a really tough thing to implement with my
  programming skills at the time, realistically. That said, I still
  think a lot of these ideas really ought to be realized by people with
  more time and energy than me.
- In 2018, [I finally caught my breath and did a remix of a song for a
  friend](https://somasis.bandcamp.com/track/boundary-ft-andr-somasis-remix).
  And then [two more remixes of one
  song](https://somasis.bandcamp.com/album/hawk-crow-somasis-remixes),
  the same year.
- In 2017, [I did a song for a Pinkamena Party
  compilation](https://pinkamenaparty.bandcamp.com/track/pagan-holiday).
  And [a two track
  single](https://somasis.bandcamp.com/album/sunraiser-revolution-means-beauty).
- In 2016, I worked on converting the old MediaWiki used by the musl
  ...
  [here](https://wiki.musl-libc.org). The tools I wrote, named
  [`makedown`](https://github.com/somasis/makedown) are available as
...
- In 2015, I put a small `curl` wrapper
  ([`wget-curl`](https://github.com/somasis/wget-curl)) that would
  translate `wget`-style arguments to their equivalents in `curl` syntax
  on GitHub. Later (and without my knowing), it was included in
  CyanogenMod for a time, and was [later
  removed](https://github.com/LineageOS/android_vendor_lineage/commit/0623e975037cd68b15966fe154af59e4de6a7e00)
  after issues with users tripping over it, since I believe the
  developers of CyanogenMod mostly started using it for replacing `wget`
  in the system tool set somewhere. I was an enthusiastic user of the
  [Toybox](https://www.landley.net/toybox/) project (an alternative to
  `busybox`) at the time, and wrote the script primarily as an
  experiment of my own, for until Toybox got its own `wget`
  reimplementation; I was in the process of trying to enable `toybox` to
  provide a `coreutils` alternative on Exherbo Linux.
  - I also lightly ported FreeBSD's `libarchive`-utilizing `unzip`
    implementation, and I called it
    [`bsdunzip`](https://github.com/somasis/bsdunzip).
- In 2015, I wrote a silly little init daemon called
  [`beginning`](https://github.com/somasis/beginning); it was a fun
  project but I eventually realized that
  [`s6`](http://skarnet.org/software/s6/) was a much more
...
- In 2015, for making it easier to deal with Gerrit, I wrote
  [`git-gerrit`](https://github.com/somasis/git-gerrit), which was a
  wonky little script to automate interacting with with Gerrit over its
  SSH frontend.
- And also in 2015 (I had a lot of free time) [I started making a
  fork...​ thing...​ of my favorite GTK theme set of the
  time](https://github.com/somasis/shiki-colors-revival), Shiki-Colors
  (also known as
  [GNOME-Colors](https://github.com/gnome-colors/gnome-colors)). I still
  use some variation on this theme, but I've not really published it
  anywhere.
- In 2015 [I did some more tracks for a Gak Attack Records
  compilation](https://somasis.bandcamp.com/album/gak-attack-vol-2), our
  final release it seems.
- At the end of 2014, I wrote a script for updating the Linux kernel
  from source, called [`lux`](https://github.com/somasis/lux), primarily
...
- In 2014, I wrote a Tumblr theme called Bootstrapped---it was actually
  in the theme gallery for a time too. [Here's what it looked
  like.](https://bootstrapped-demo.tumblr.com/)
- When 2014 came, and I made [an
  EP](https://somasis.bandcamp.com/album/acetate-2014), [an
  EP](https://somasis.bandcamp.com/album/the-infinite-sadness-2), and
  what I felt was {url-music-tapedeck-spaceship}\[my first actually good
  album-length album\]. Then for some reason [I released some rejects
  from that
  album](https://somasis.bandcamp.com/album/rejected-from-the-tapedeck-spaceship).
- In 2013, [I made some of my first
  music](https://somasis.bandcamp.com/track/3-48-shark-somasis-silence-massive-get-hype-by-somasis).
  Then [an EP](https://somasis.bandcamp.com/album/sleepless). Then, my
  first
  [album](https://somasis.bandcamp.com/album/the-lonesome-planetarium).
  Then [another album](https://somasis.bandcamp.com/album/talent-show).
  [Some
  remixes](https://somasis.bandcamp.com/album/talent-show-the-remixes).
  And a
  [silly](https://somasis.bandcamp.com/album/im-no-jungalist-massif) EP.
  [A song and a half for a Pinkamena Party
  album](https://pinkamenaparty.bandcamp.com/album/rainbow-dash-impales-herself-on-a-wrought-iron-fence).
  [My first reasonably good
  album](https://somasis.bandcamp.com/album/a-most-beautiful-sight) (I
  think it's my girlfriend's favorite). A song that would be on a later
  2014 album ([\"Live from the Tapedeck
  Spaceship\"](https://somasis.bandcamp.com/album/live-from-the-tapedeck-spaceship)),
  came out on [Pinkamena Party's third
  compilation](https://pinkamenaparty.bandcamp.com/track/pagan-holiday).
  [And then one last
  EP](https://somasis.bandcamp.com/album/scattered-we-unified). Then, I
  worked with some friends from the Brony music scene on an
  IDM/breakbeat hardcore netlabel called Gak Attack Records, and we did
  that thing computer musicians do when they come together---[we made a
  compilation](https://somasis.bandcamp.com/album/gak-attack-vol-1).
- Back in 2012, I wrote a set of scripts for a Minecraft server plugin
  called CommandHelper, called [`rccmd`
  (mirrored)](https://github.com/LadyCailin/RCcmd), which provided a set
  of general commands for server users and administrators, as an
  alternative to more full-featured command set plugins of the time.
- Back in 2011 (I think), I made a GNOME Shell theme called Seeha, but I
  can't find an archive of it anywhere. [There was an AUR package for
  it, though](https://github.com/aur-archive/gnome-shell-theme-seeha).
  It was put on DeviantArt, but in the years since, I deleted my
  account.
- In 2011, I made a custom theme for the Wii menu, called
  ...
  people with little coding experience. The theme download still works,
  if you can figure out how to install it!
- My most obscure flex is that [in 2010 I got published in
  OMGubuntu](https://www.omgubuntu.co.uk/2010/08/meet-alawalk-possibly-the-lightest-launcher-ever)
  for a dinky little shell script launcher program called Alawalk, [and
  its
  update](https://www.omgubuntu.co.uk/2010/08/super-light-launcher-alawalk-0-4-released-boasts-21-plug-ins),
  and [then that launcher program got
...
- [linkedin](https://www.linkedin.com/in/kylie-mcclain/)
- [steam: somasis](https://steamcommunity.com/id/somasis/)
- [last.fm: kyliesomasis](https://last.fm/user/kyliesomasis)
- [listenbrainz: somasis](https://listenbrainz.org/user/somasis/)
- [openstreetmap: somasis](https://www.openstreetmap.org/user/somasis)
