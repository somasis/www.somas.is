title: TODO: building a commune: Mutiny
summary: an overview of my research tech project, Mutiny, with historical discussion.
...
I called it [Mutiny][mutiny], because I was hoping to channel the feel of a sort of "computer
...
perfection. That is to say, it's basically [Zen and the Art of Motorcycle Maintenance][zen][^zen].
...
: The Passively Capricious Zebrapig, Exherbo's mascot, who I love with all my heart.
  ![A funny little drawing of a pig with zebra colors.](note-mutiny-zebrapig.png)

[dev]: https://git.exherbo.org/gitolite-admin.git/commit/?id=bb3484db0737b26e64d7ff4207df8930c6523185

After mastering Exherbo, eventually [I was recruited as a developer for the distribution][dev],
...
[philosophy]: https://www.openbsd.org/goals.html

At some point or another, I found myself absolutely _enamoured_ with OpenBSD's
[philosophy]. The idea of one system all under the BSD license, I can take the code as I
...
[^virtuals]:
    Virtuals, if you're not familiar, are essentially dummy packages with a "provider" set for them,
    i.e. `virtual/man` may have `providers:mandoc` set or `providers:man-db` set, because they
    provide equivalent functionality and don't change anything at package build time.
    It just makes specifying dependencies easier.

At some point, I really started to diverge in goals from my fellow Exherbo developers. I had been
adding more "virtuals"[^virtuals] to Exherbo's package set, for things that were previously deemed
trivial: `wget`, `awk`, `sed`, all of these core system utilities, because I wanted to use
[Busybox] rather than [GNU coreutils]. This caused some light tension,
...
My hope was I would take ideas I learned from [Exherbo], and other ideas from new
distributions springing forth, like [Sabotage Linux], [Morpheus Linux],
and the (now) most popular of them all, [Alpine Linux].

[^elfutils]:
    I actually got yelled at for adding these patches; I very boldly applied a musl-only patch to
    elfutils, and ended up breaking everyone's copy of it on their glibc machines.

Where did all these weird little distributions come from? Developers getting interested in new
technology, as usual. [musl libc], an alternative implementation of the C standard library by
Rich Felker and others was finally getting close to being a usable piece of software for the average
desktop and server administrator. [Patches][patch1] [would][patch2] [still][patch3] [be][patch4]
required[^elfutils] [for][patch5] [almost][patch6] [all][patch7] [low-level][patch8]
[packages][patch9], but it's surprising how far you could actually get that early on.
...
[Bitrig] back when they were a thing), but either due to hardware issues, a lack of familiarity, or
just a preference of Linux's chaotic style, they didn't click either.
...
* The init system that we're starting with is [s6] + [s6-rc], but the goal is that the system will
  be as flexible as possible to allow for any init system, any libc, any toolchain, and so on.
...
* Support multiple build targets in an integrated way.
    * This was another unusual choice. Exherbo had recently added a ["cross" functionality][cross]
      into the package repositories and the package manager. The idea in short, is that rather than
      the classic x86/amd64 division that other distributions use, such as [Debian's "multiarch"]
      and [Gentoo's "multilib"][multilib], each architecture is treated as a separate
...
* Adhere to a new philosophy, influenced by BSD hacker ethic and Linux-style openness. Have
  ...
  "ports". Decentralized programs come together to create the user's system.
    * This was another unusual idea. Part of what made me admire BSD systems so heavily, and part
      ...
      sort of synthesis of these two [historically opposed][histopp] ways of development.
...
Fig. 2:
: The [typeface for Mutiny][type], which has been around since I made the original design draft.
  ![Mutiny logo](https://mutiny.red/logo.svg)
